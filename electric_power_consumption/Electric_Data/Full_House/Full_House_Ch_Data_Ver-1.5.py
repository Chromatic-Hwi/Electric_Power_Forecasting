...
for Total_num in range(3, 54):
    try:
        ch = pd.read_csv('./channel_'+str(Total_num)+'.dat', names=['Unix', 'Watt'],header=None, delimiter=" ")
        print('Dat File Read Done!\n')
        ch.info()

        ch_date = pd.DataFrame(ch,columns = ['Unix', 'Date', 'Watt'])

        ch_date['Date'] = pd.to_datetime(ch['Unix'], unit='s')
        ch_date.info()
        print()
        ch_date.head(10)
        print()
        ...
        try:
            os.mkdir('./Ch'+str(Total_num)+'_Electirc_Data_CSV')
            print('폴더 생성 완료\n')
        ...
        # 2. 
        while(i < len(ch_date)):
            list1 = ch_date.loc[i].values.tolist()
            date = list1[1]
            y1 = date.year
            M1 = date.month
            d1 = date.day
            ...
            while(H1 < 24):
                ...
                while(H0 < 24):
                    ...
                    if(H1 == H0):
                        ...
                        while(h < h+1):
                            n=n+1
                            list2 = ch_date.loc[i].values.tolist()
                            date = list2[1]
                            watt = list2[2]
                            y = date.year
                            M = date.month
                            ...
                            # 3.1'.4.
                                ...
                                with open("./Ch"+str(Total_num)+"_Electirc_Data_CSV/"+str(y4)+'_'+str(M4)+'_'+"House1_Ch"+str(Total_num)+"_Electric_Data.csv", 'a', newline='') as file:
                                    writer = csv.writer(file)
                                    writer.writerow(hourly_list)       
                                        ...
                                        if(int(M) != int(M4)):
                                            ...
                                            while(D2 < d):
                                                for x in range (0,24):
                                                    ...
                                                    with open("./Ch"+str(Total_num)+"_Electirc_Data_CSV/"+str(y4)+'_'+str(M)+'_'+"House1_Ch"+str(Total_num)+"_Electric_Data.csv", 'a', newline='') as file:
                                                        writer = csv.writer(file)
                                                        writer.writerow(hourly_list)  
                    ...
                    # 3.2. 
                    if(H1 != H0):
                        list3 = ch_date.loc[i].values.tolist()
                        date = list3[1]
                        watt = list3[2]
                        y = date.year
                        M = date.month
